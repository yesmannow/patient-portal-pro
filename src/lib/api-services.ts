...
export interface DrugInfo {
  ...
  warnings: string[]
  recalls: any[]
  adverseEvents: Array<{ symptom: string; frequency: string }>
}
...
export class APIServices {
  ...
  static async validatePhone(phoneNumber: string): Promise<PhoneValidationResult> {
    if (this.MOCK_MODE) {
      const mobilePatterns = ['555-123', '555-234', '555-345', '555-456']
      const isMobile = mobilePatterns.some(pattern => phoneNumber.includes(pattern))
      return {
        valid: phoneNumber.length >= 10,
        number: phoneNumber,
        lineType: isMobile ? 'mobile' : 'landline',
        carrier: isMobile ? 'Verizon Wireless' : 'AT&T Landline',
    ...
    try {
      const response = await fetch(`https://api.numverify.com/validate`, {
        method: 'POST',
