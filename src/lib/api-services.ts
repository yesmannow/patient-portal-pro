...
export class APIServices {
  ...
  static async validatePhone(phoneNumber: string): Promise<PhoneValidationResult> {
    if (this.MOCK_MODE) {
      const isMobile = phoneNumber.includes('555') || phoneNumber.includes('123')
      return {
        valid: phoneNumber.length >= 10,
        number: phoneNumber,
        lineType: isMobile ? 'mobile' : 'landline',
        carrier: isMobile ? 'AT&T Wireless' : 'Verizon',
    ...
    try {
      const response = await fetch(`https://numverify.com/api/validate`, {
        method: 'POST',
